      ...
      esivis.find('#iframe')[0].onload = function () {
        ...
        beefApp.frame.find('.text p').each(function () {
          var sentences = $(this).text().replace(/([^.!?]*[^.!?\s][.!?]['"]?)(\s|$)/g, function (val) {
            i++;
            return '<div class="side_viz side_vis_sentence_' + i + '"></div><span class="sentence sentence_' + i + '" data-sentence="' + val.trim() + '" data-sentence-class="sentence_' + i + '" data-sentence-id="' + i + '"><a href="javascript:;">' + val + '</a></span>';
          });
          $(this).html(sentences);
        });
